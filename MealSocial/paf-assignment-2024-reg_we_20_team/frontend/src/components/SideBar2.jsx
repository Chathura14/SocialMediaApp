...
const SideBar2 = ({ logUser }) => {
  ...
  <div className="flex flex-col min-h ">
    <aside className="right-0 fixed lg:w-[250px] w-[250px] border-r border-r-dashed border-r-neutral-100 p-4 bg-red-600 rounded-lg shadow-md">
      ...
      <div className="flex flex-col gap-5 ml-1 text-sm text-neutral-200 ">
        {users?.map((user, index) => {
          return logUser?.id !== user?.id ? (
            ...
            <div
              key={index}
              className="flex items-center gap-5 cursor-pointer bg-transparent hover:bg-red-500 rounded-lg pl-2 pt-1 pb-1 pr-2 "
